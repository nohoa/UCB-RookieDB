...
import static edu.berkeley.cs186.database.index.InnerNode.numLessThanEqual;
...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int B_tree_order = metadata.getOrder();
        //System.out.println("BTree order is: "+ B_tree_order);
        int found_index = -1;
        ...
        if (found_index == -1) {
            found_index = keys.size();
        }
        //System.out.println("Id is: " + found_index);
        keys.add(found_index,key);
        rids.add(found_index,rid);
        if(keys.size() <= 2*B_tree_order) {
            //return Optional.empty();
            sync();
            return Optional.empty();
        }
        List<DataBox> left_key = new ArrayList<>() ;
        List<DataBox> right_key = new ArrayList<>();
        List<RecordId> left_id = new ArrayList<>() ;
        List<RecordId> right_id = new ArrayList<>() ;

        for(int i = 0 ;i < B_tree_order ;i ++){
            left_key.add(keys.get(i));
            left_id.add(rids.get(i));
        }

        for(int i = B_tree_order ;i < keys.size() ;i ++){
            right_key.add(keys.get(i));
            right_id.add(rids.get(i));
        }

        keys = left_key;
        rids = left_id;

        LeafNode new_leaf = new LeafNode(metadata,bufferManager,right_key,right_id,rightSibling,treeContext);
        Long page_num = new_leaf.getPage().getPageNum();
        rightSibling = Optional.of(page_num);
        sync();

        return Optional.of(new Pair<>(right_key.get(0),page_num));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
            if (keys.size() < Math.ceil(fillFactor * metadata.getOrder()* 2 )) {
                for (int i = keys.size() + 1; i < Math.ceil(fillFactor * metadata.getOrder()* 2 ) + 1 && data.hasNext(); i++) {
                ...
                Pair<DataBox, RecordId> current_data = data.next();
                keys.add(current_data.getFirst());
                rids.add(current_data.getSecond());
            List<DataBox> left_key = new ArrayList<>();
            List<DataBox> right_key = new ArrayList<>();
            List<RecordId> left_id = new ArrayList<>();
            List<RecordId> right_id = new ArrayList<>();
            ...
            right_key.add(current_data.getFirst());
            right_id.add(current_data.getSecond());

            keys = left_key;
            rids = left_id;
           // System.out.println("Here come leaf check :");
//            for(int i = 0 ;i < keys.size() ;i ++){
//                System.out.print(keys.get(i) +" ");
//                System.out.print(rids.get(i) +" ");
//            }
           // System.out.println("\n");
            LeafNode newLeaf = new LeafNode(metadata, bufferManager, right_key, right_id, rightSibling, treeContext);
            ...
            return Optional.of(new Pair<>(current_data.getFirst(), pageNum));
        }
    }
    ...
    public void remove(DataBox key) {
        ...
        if(keys.get(index -1).compareTo(key) == 0) {
            keys.remove(index-1);
            rids.remove(index-1);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        //System.out.println("Right Sibling value is : " + rightSibling);
        ...
        //System.out.println("Size of keys : "+ n) ;

        for(int i = 0 ;i < n ; ++i){
            DataBox curr_key = DataBox.fromBytes(buf, metadata.getKeySchema());
            long pageNo = buf.getLong() ;
            short entryNo = buf.getShort();
            RecordId id = new RecordId(pageNo,entryNo);
            keys.add(curr_key);
            rids.add(id);
        }
